		...
		foreach ($mostrar as $key => $value) {
			...
			if ($value["TIPO_CAMION"] == "Camion") {
				...
				if ($value["ESTATUS_CAMIONES"] == "Disponible") {
					echo '<button id="' . $value["ID_CAMIONES"] . '" idEstatus="' . $value["ID_CONDUCTORES"] . '" value="" class="btn btn-xs btn-success estatusCamiones">' . $value["ESTATUS_CAMIONES"] . '</button>';
				}
				if ($value["ESTATUS_CAMIONES"] == "Mantenimiento") {
					echo '<button id="' . $value["ID_CAMIONES"] . '" idEstatus="' . $value["ID_CONDUCTORES"] . '" value="" class="btn btn-xs btn-warning estatusCamiones">' . $value["ESTATUS_CAMIONES"] . '</button>';
				}
				if ($value["ESTATUS_CAMIONES"] == "Cargado") {
					echo '<button id="' . $value["ID_CAMIONES"] . '" idEstatus="' . $value["ID_CONDUCTORES"] . '" value="" class="btn btn-xs btn-primary estatusCamiones">' . $value["ESTATUS_CAMIONES"] . '</button>';
				}
				if ($value["ESTATUS_CAMIONES"] == "Repartiendo") {
					echo '<button id="' . $value["ID_CAMIONES"] . '" idEstatus="' . $value["ID_CONDUCTORES"] . '" value="" class="btn btn-xs btn-danger estatusCamiones">' . $value["ESTATUS_CAMIONES"] . '</button>';
				}
				echo '</td>					
						<td>
							' . $value["TIPO_CAMION"] . '
